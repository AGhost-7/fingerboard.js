...
class View {
  ...
  pinpointNote(x, y) {
    ...
    const fret =
      x < openFretWidth ? 0 : Math.floor((x - openFretWidth) / fretWidth) + 1
  ...
  onMouseMove(e) {
    ...
    if (
      !this.hoveredNote ||
      this.hoveredNote.fret !== fret ||
      this.hoveredNote.string !== string
    ) {
  ...
  paint() {
    ...
    const heightRatio = this.height / this.model.strings
    const radius =
      heightRatio > openFretWidth ? openFretWidth / 4 : heightRatio / 4
